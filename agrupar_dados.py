...
def preprocess_particles(listP, data):
    # Separando as partículas pelas suas categorias 
...
def concatenando_dados(data, feature_names):
    dfs_concatenados = []
    CATEGORIAS = len(data)
    print(f"Categorias: {CATEGORIAS}")

    for k in range(CATEGORIAS):
        # Definindo a classe de acordo com a categoria
        if k == 0:
            classe = 0  # data_q
        elif k == 1:
            classe = 1  # data_g
...
def executando(path,caminho_saida):
    ...
    # Agora mantemos como uma lista de arrays, ao invés de forçar um np.array
    data_P = [data_q, data_g, data_Z, data_t, data_W]  # Lista ao invés de np.array
    print(data_P)

    df = concatenando_dados(data_P, label)
    util.salvar_tensor(df, caminho_saida)
